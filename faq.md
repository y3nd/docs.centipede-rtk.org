...
> No! It requires some hardware (therefore some money), a bit of Internet connectivity (3–4G mobile and about 5 MB/hour), and some understanding of the technology to use it effectively.
...
> In the case of the Centipede-RTK project, the idea is to use generic components and open-source software, so the costs are divided by 10, around €250–300 for hardware (without options), whether it is a Base or a Rover.
...
> You need hardware compatible with L1 or L1-L2 satellite signals.

* ### What is a base and a rover?

> They use exactly the same components (receiving antenna + decoding chip + nano computer + software), but they have different functions.
...
> * A **rover** is a set of components that receives signals from navigation satellites (GPS + GLONASS + GALILEO + ...) and receives correction data from a base (via mobile 3–4G or radio). This setup makes it possible to calculate a geometric position with centimeter-level accuracy depending on the conditions.
...
> Yes, we use mobile technology (3–4G) to retrieve this data in the field. Therefore, we use an existing transmission.

> No, we deliberately chose this technology to avoid introducing additional radio emissions through the purchase of a dedicated transmission frequency. We therefore use existing infrastructure, with its advantages and disadvantages. [coverage gaps](https://www.arcep.fr/cartes-et-donnees/nos-publications-chiffrees/observatoire-des-deploiements-mobiles-en-zones-peu-denses/les-deploiements-mobiles-dans-les-zones-peu-denses.html){:target="_blank"}
...
> The network is community-based for reception, so no login or password is required.

> To declare an RTK base for transmission, it is necessary to carefully follow the instructions on the next pages of this site.
